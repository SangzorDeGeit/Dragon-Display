...
pub fn remove_campaign_from_config(campaign: Campaign) -> Result<(), io::Error> {
    ...
    let index = match new_campaign_list.iter().position(|c| c.name == campaign.name) {
        Some(i) => i,
        None => return Err(Error::new(ErrorKind::NotFound, "Could not find the campaign to be deleted in the config folder")),
    };
...
fn check_integrity(config: &Config) -> Result<(), io::Error> {
    ...
    for campaign in &config.campaigns {
        ...
        if path_names.contains(&path) {
            return Err(Error::new(ErrorKind::InvalidData, "Multiple campaigns were found with the same image folder path"));
        }
...
fn check_save_removal(campaign_path: &str) -> Result<(), io::Error> {
    let files = match fs::read_dir(&campaign_path) {
        ...
        Err(e) => {
            match e.kind() {
                ...
                ErrorKind::PermissionDenied => return Err(Error::new(ErrorKind::PermissionDenied, "Insufficient permissions to find the image folder of the campaign")),
                _ => return Err(Error::new(ErrorKind::Other, "An unexpected error occured while trying to find the image folder of the campaign")),
            }
        }
    };
    ...
    for file in files {
        ...
        let extension = match extension_os.to_str() {
            Some(e) => e,
            None => return Err(Error::new(ErrorKind::NotFound, "Could not convert file types into strings")),
        };

        if !IMAGE_EXTENSIONS.contains(&extension) {
            return Err(Error::new(ErrorKind::NotFound, "Could not remove image folder: found non-image files in the campaign image folder"));
        }
    }
